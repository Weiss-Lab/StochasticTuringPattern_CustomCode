function trendGraph
%sample each chemical serpately
writefile='/home/michael/turingW/direct/individualChem4.txt';
writefile2='/home/michael/turingW/direct/resultsChem4.txt';
res=fopen(writefile, 'a');
res2=fopen(writefile2, 'a');
sampleNum=101;
Dt=.5;
D2t=Dt*21.6;
...
namesChem={'D'; 'D2'; 'gu'; 'gv'; 'gc'; 'giu'; 'giv'; 'au'; 'av'; 'aiu'; 'aiv'; 'ac'; 'lam_u'; 'lam_v'; 'fod_1'; 'fod_2'; 'fod_3'; 'fod_4'; 'fod_5'; 'fod_6'; 'Kd_1'; 'Kd_2'; 'Kd_3'; 'Kd_5'; 'Kc_3'; 'Kd_4'; 'Kd_6'; 'Lac'; 'IPTG'};

hold on

for t=1:29

maxV=[Dt D2t gu gv gc giu giv au av aiu aiv ac lam_u lam_v fod_1 fod_2 fod_3 fod_4 fod_5 fod_6 Kd_1 Kd_2 Kd_3 Kd_5 Kc_3 Kd_4 Kd_6 Lac IPTG]; %maximium Value in range
minV=[Dt D2t gu gv gc giu giv au av aiu aiv ac lam_u lam_v fod_1 fod_2 fod_3 fod_4 fod_5 fod_6 Kd_1 Kd_2 Kd_3 Kd_5 Kc_3 Kd_4 Kd_6 Lac IPTG]; %minimium value in range
maxV(t)=maxV(t)*1.5;
minV(t)=minV(t)*0.5;
...
for n =1:sampleNum
    ...
    Chemical(n)=values(n,t)/(maxV(t)-minV(t));
    patternType(n)=pat;
    waveNumber(n)=kmin1;
    waveNumber2(n)=kmin2;
    waveNumberE(n)=kminE;
...
xp2=t;
yp1=0;
yp2=maxPower(sampleNum)-maxPower(1);
...
line([xp1 xp2],[yp1 yp2],'Color', colorV, 'LineWidth',10);

% titPat=sprintf('Pattern Type as a Function of %s%', namesChem{t});
% xL=sprintf('%s', namesChem{t});
% figure(1), plot(Chemical, patternType, '.'), title(titPat),xlabel(xL),ylabel('Pattern Type');
% titPic=sprintf('%sPattern',namesChem{t});
% print('-f1',titPic,'-dpng')
% titPat=sprintf('Wavenumber vs %s%', namesChem{t});
% xL=sprintf('%s', namesChem{t});
% figure(2), plot(Chemical, waveNumber,'.', Chemical, waveNumber2,'.', Chemical, waveNumberE,'.'), title(titPat),xlabel(xL), ylabel('Wavenumber k');
% titPic=sprintf('%sWavenumber',namesChem{t});
% print('-f2',titPic,'-dpng')
% titPat=sprintf('Max Eigenvalue for %s%', namesChem{t});
% xL=sprintf('%s', namesChem{t});
% figure(3), plot(Chemical, maxPower,'.'), title(titPat),xlabel(xL),ylabel('Max Eigenvalue');
% titPic=sprintf('%sEigenvalue',namesChem{t});
% print('-f3',titPic,'-dpng')
% number0
% number1
% number2
% number3
...
% number4/sampleNum
fprintf(res2, '%s\n', namesChem{t});
fprintf(res2, '%d %d %d %d %d\n',number0,number1,number2,number3,number4);
fprintf(res2, '%g %g %g %g %g\n',number0/sampleNum,number1/sampleNum,number2/sampleNum,number3/sampleNum,number4/sampleNum);
end

line([0 21],[0 0],'Color','black');

hold off
%axis([0 21 0 1]);
ax = gca;
set(ax,'XTick',1:29);
set(ax,'XTickLabel',namesChem(1:29));
ylabel('Difference in Eigenvalues');
%set(ax,'YTick',[]);
title('Sensitivity');
fclose(res);
fclose(res2);
