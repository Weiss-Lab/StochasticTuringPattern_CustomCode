...
%Analyze domain characters
    ...
    med_areaFRS = median(alldomainsFRS);  % Find the median area in the filtered image.
    disp(['med_area(filRS)= ' num2str(med_areaFRS)]);
    std_areaFRS = std(alldomainsFRS);  % Find the standard deviation area in the filtered image.
